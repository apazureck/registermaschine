...
export function findInstructionfromOpcode(opcode: string) {
  let t = REGISTERMASCHINE_INSTRUCTIONS.filter(
    (instruction) => instruction.opcode === opcode.toUpperCase()
  )[0];
  console.log(t);
  return t;
}
...
export const REGISTERMASCHINE_PROGRAM_KEYWORDS =
  REGISTERMASCHINE_INSTRUCTIONS.filter(
    (instruction) => instruction.category === 'program'
  ).map((instruction) => instruction.opcode);

export const REGISTERMASCHINE_MEMORY_KEYWORDS =
  REGISTERMASCHINE_INSTRUCTIONS.filter(
    (instruction) => instruction.category === 'memory'
  ).map((instruction) => instruction.opcode);

export const REGISTERMASCHINE_OTHER_KEYWORDS =
  REGISTERMASCHINE_INSTRUCTIONS.filter(
