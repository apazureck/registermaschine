...
describe('AddCommand', () => {
  it('should add value from data memory to accumulator', () => {
    ...
    const command = new AddCommand(registermaschine, 'ADD 2', 1, 1);
    command.load();
    command.execute();
  ...
  it('should throw an error for invalid operand', () => {
    const registermaschine = new Registermaschine();
    const command = new AddCommand(registermaschine, 'ADD X', 1, 1);
  ...
  it('should add value from accumulator, if address is zero', () => {
    ...
    const command = new AddCommand(registermaschine, 'ADD 0', 1, 1);
    command.load();
    command.execute();
